...
var Connector = require('./src/index.coffee');
  ...
  if (err) {
    ...
    client.getTwin(function (err, twin) {
      if (err) {
        ...
        // Create a meshblu device-like structure from the twin - this
        // assumes the twin has been created with an "options" object
        // in the "desired" properties.
        var device = {};
        if ('options' in twin.properties.desired) {
          device['options'] = twin.properties.desired.options;
        }
        else {
          device['options'] = {}
        }
        var myConnector = new Connector();
        myConnector.start(device, function() {});
